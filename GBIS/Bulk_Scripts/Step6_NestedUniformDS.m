function [loadedData, Filename, Filename_Raw, nObs_SS, nObs_Raw, BoundingBox, NewSS_Factor, NewSS_FactorF] = Step6_NestedUniformDS(Phase,lon,lat,FineBoundingBox,head,incidence,VolcName,Frame,largest_component_mask,SignalLocation,Options)
    ...
    for k = 1:length(SS)
        ...
        elseif k==2
            ...
            save(Filename,'Lon','Lat','Phase','Inc','Heading','Offset','FineBoundingBox','CoarseIdxs','FineIdxs');
        end
        ...
        if SS(k) == 'y'
            ...
            OtsuDSFigFilename = strcat(pwd,'/Bounding_Boxes/',VolcName,'OtsuDSFig','_Shape_',num2str(Options.Otsu_BB_Shape),Extra,'_',Options.RunID,'.png');
            saveas(OtsuOverview,OtsuDSFigFilename);
            save([pwd,'/Bounding_Boxes/',VolcName,'_Shape_',num2str(Options.Otsu_BB_Shape),Extra,'BoundingBox.mat'],'FineBoundingBox','FineBoundingBoxLL','BoundingBox','BoundingBoxPoly','largest_component_mask');
            save([pwd,'/Spatial_Parameters/',VolcName,'_Shape_',num2str(Options.Otsu_BB_Shape),Extra,'_SignalLocation.mat'],'SignalLocation');
            save([pwd,'/nObs/',VolcName,'_',SS_Style,'_',Options.RunID,'_nOBS.mat'],'nObs_SS','nObs_Raw','nObs_SS_Fine','nObs_SS_Coarse','DS_Stats');
        end
    end
end