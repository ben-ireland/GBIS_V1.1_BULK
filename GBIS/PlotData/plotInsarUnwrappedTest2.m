function plotInsarUnwrappedTest2(xy, los, cmap, name)
...
%% Patch scattered data for faster plotting
    edge = round(((min(abs(diff(xy(:,2)))))+1))+2; % Size of patch set top minumum distance between points %Edited
    if edge < 50
        edge = 50;
    end
    xs = [xy(:,1)'; xy(:,1)'+edge; xy(:,1)'+edge; xy(:,1)']; % Coordinates of four vertex of patch
    ys = [xy(:,2)'; xy(:,2)'; xy(:,2)'+edge; xy(:,2)'+edge];
...
% Display unwrapped interferogram
    ...
    %t = title(['Unwrapped InSAR Data: ', name],'FontSize', 18,'Interpreter','none');
    %set(t,'Position',get(t,'Position')+[0 1000 0]);

    saveas(fig,'TestDMR.png');