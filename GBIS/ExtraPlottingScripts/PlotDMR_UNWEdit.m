function [Fig, RMS] = PlotDMR_UNWEdit(OutputFilePath)
...
for i=1:length(insar)
    ...
    nexttile
    % DATA
    scatter(ll(:,1), ll(:,2),5,los,'square', 'filled');
    colormap(jet); 
...
%     sgtitle(outputFileName(1:end-4),'interpreter','none');
    ...
    nexttile
    % MODEL
    scatter(ll(:,1), ll(:,2),5,modLos2,'square', 'filled');
    colormap(jet)
    c = max(abs([min(los), max(los)])); % Calculate maximum value for symmetric colormap
    caxis([-c c])
    axis square
...
%     title('Model','interpreter','none')
    ...
    nexttile
    % RESIDUAL
    scatter(ll(:,1), ll(:,2),5,Residual,'square', 'filled');
    colormap(jet); 
    c = max(abs([min(los), max(los)])); % Calculate maximum value for symmetric colormap
    caxis([-c c])
    axis square
