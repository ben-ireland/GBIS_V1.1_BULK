function ULos = forwardInsarModelV2(insar,xy,invpar,invResults,modelInput,geo,Heading,Inc,constOffset,xRamp,yRamp,idx)
...
paramSource = invResults.mKeep(:,idx);

for i = 1:invpar.nModels % For each source model...
    index1 = invResults.model.mIx(i);
    switch invpar.model{i}
        case 'MOGI'
            mFunc{i} = paramSource(index1:index1+3);
            U = mogi(mFunc{i},xy,nu);
        case 'MCTG'
            mFunc{i} = paramSource(index1:index1+4);
            U = mctigueSource(mFunc{i},xy(1:2,:),nu);
        case 'YANG'
            mFunc{i} = paramSource(index1:index1+7);
            U = yangSource(mFunc{i},xy,nu);
        case 'PENN'
            mFunc{i}= paramSource(index1:index1+4);
            U = pennySource(mFunc{i},xy,nu);
        case 'SILL'
            mFunc{i}=[paramSource(index1:index1+2);0; ...
                paramSource(index1+3:index1+5);0;0; ...
                paramSource(index1+6)]; 
            U = disloc(mFunc{i},xy(1:2,:),nu); 
        case 'DIKE'
            mFunc{i}=[paramSource(index1:index1+6);0;0;...
                paramSource(index1+7)]; 
            U = disloc(mFunc{i},xy(1:2,:),nu); 
        case 'FAUL'
            mFunc{i}=[paramSource(index1:index1+8);0]; 
            U = disloc(mFunc{i},xy(1:2,:),nu); 
        case 'HING'
            mFunc{i}=paramSource(index1:index1+10);
            U = hingedDikes(mFunc{i},xy(1:2,:),nu);
        case 'SUNC'
            mFunc{i}=paramSource(index1:index1+4); 
            U = Sun1969Source(mFunc{i},xy(1:2,:),nu); 
        case 'SUNV'
            mFunc{i}=paramSource(index1:index1+4); 
            U = Sun1969VolSource(mFunc{i},xy(1:2,:),nu); 
        case 'CERV'
            mFunc{i}=paramSource(index1:index1+7);
            U = CervelliYangVolume(mFunc{i},xy,nu);  
        case 'CERP'
            mFunc{i}=paramSource(index1:index1+7);
            U = CervelliYangPressure(mFunc{i},xy,nu);  
        case 'CDMN'   
            mFunc{i}=paramSource(index1:index1+9);
            U = PrepCDM(mFunc{i},xy,nu); 
        case 'CDMB'   
            mFunc{i}=paramSource(index1:index1+4);
            U = CDM_SymmetricSimple(mFunc{i},xy,nu);
        case 'CDMG'   
            mFunc{i}=paramSource(index1:index1+6);
