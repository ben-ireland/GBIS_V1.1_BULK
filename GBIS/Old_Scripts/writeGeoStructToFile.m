function writeGeoStructToFile(Struct, filename)
%Function to create .inp text input file for GBIS (Bagnardi and Hooper,
%2018) from a 'geo' structure.
...
for i = 1:length(fields)
   ...
   elseif isnumeric(s.(field_name)) && numel(s.(field_name)) > 1
          field_values{end+1} = {field_name,['[', sprintf('%.2f; ', s.(field_name)), sprintf('%.2f', s.(field_name)(end)), ']']};
    else
