function name = plotInSARDataModelResidualPDF(insar, geo, invpar, invResults, modelinput, saveName, saveflag)
...
for i=1:length(insar)
    ...
    [pathstr,name{i},ext] = fileparts(insar{i}.dataPath);
   ...
   if i > 1
    ...
    img = getframe(gcf);
    if saveflag=='y'
        imwrite(img.cdata,[outputDir,'/','Summary_Figures/',inputFileN,'/',saveName,'/InSAR_Data_Model_Residual_',name{i},num2str(i),'.png']); %Edited
        
        save([outputDir,'/Residual/Residual_',name{i},saveName,'.mat'],'residual','xy'); %Added
        save([outputDir,'/Model/Model_',name{i},saveName,'.mat'],'modLos','xy'); %Added
    end
