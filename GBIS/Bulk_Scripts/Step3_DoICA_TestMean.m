function [Phase] = Step3_DoICA_TestMean(TS_File, LastStep, Location, Options)
...
for a = 1:length(ncomp)
    ...
    while (mean(Closeness(:)) < Options.ICA_ClosenessThresh) | (mean(Closeness(:)) > MaxCloseness)
        b = b + 1;
        if sign(MaxFullRe) ==1
    ...
    ClosenessAll(a) = mean(Closeness(:));

    if mean(Closeness(:)) <= Options.ICA_ClosenessThresh
        ...
        continue
   elseif mean(Closeness(:)) >= MaxCloseness
...
if (ClosenessAll(end) <= Options.ICA_ClosenessThresh) || (ClosenessAll(end) >= MaxCloseness)
    disp('All ncomp values exhausted - revert to using original deformation data')
    Options.ICA = 0;
    Outcome = '>1-Thresh% residual signal';
    Note = 'Using original data';
else
    Options.ICA = 1;
    Outcome = '<1-Thresh% residual signal';
    Note = 'Using ICA outputs';
end
...
save([pwd,'/ICA/ICA_',TS_File.name(1:end-3),'_',Options.RunID,'Comparison.mat'],'Original_Disp','LastStepEachIC','ICA_Reconstructed_Disp','ResidualICA','ICA_Tests','Note');
...
if Options.ICA ==1
    ...
    if Options.IgnoreLastStep ==0
        ...
        LastStepICA = ICAs_Reconstructed(:,:,end-1) - ICAs_Reconstructed(:,:,2);
    end
    keyboard
