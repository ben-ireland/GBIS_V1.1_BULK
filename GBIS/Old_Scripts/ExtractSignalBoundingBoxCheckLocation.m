function [BoundingBox, Image, OtsuFigure, SignalLocation, largest_component_mask] = ExtractSignalBoundingBoxCheckLocation(BB_Shape, LastStep, numlevels, VolcName, SpatialRes, lon, lat, Location, Options)
...
for i=1:numlevels
    ...
    c.Label.String = 'Region number';
        ...
        if i==numlevels
            ...
            while ~matches(VolcanoOfSignal, IfgVolcano) | LocationMatch~=1;
                a=a+1;
                [~, ind] = maxk(AvgLOS(:),a);
                ...
                for k = 1:length(unique_labels)
                    if component_sizes(k) > 100
                        AvgLOS2 = mean(abs((LastStep(labeled_array==unique_labels(k)))));

                        if AvgLOS2 > AvgLOSOverall
                            AvgLOSOverall = AvgLOS2;
                            largest_component_label = unique_labels(k);
                        end
                    end
                end
                ...
                elseif BB_Shape ==2
                    ...
                    if Options.SlidingWindow ==1
                        % Check if polygon encompasses the location found by sliding window approach
                        LocationMask = poly2mask(Location.pgon.Vertices(:,1),Location.pgon.Vertices(:,2),size(LastStep,1),size(LastStep,2));
                        % Compute union of the two masks to see if they overlap fully
                        MaskAreaBefore = sum(largest_component_mask(:));
                        largest_component_maskAft = largest_component_mask;
                        largest_component_maskAft(LocationMask==1) = 1;
                        MaskAreaAfter = sum(largest_component_maskAft(:));

                        if MaskAreaBefore==MaskAreaAfter
                            % Keep this run
                            LocationMatch=1;
                        end
                    else
                        LocationMatch=1; % So this expression is already met if the locations cannot be compared
                    end  
                if a==5
