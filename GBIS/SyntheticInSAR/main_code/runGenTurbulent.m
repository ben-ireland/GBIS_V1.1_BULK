% This code is for generate turbulent atmospheric delay

% clear all

addpath('Synthetic_InSAR_image-main/turbulent/');

outputRoot = '/Users/jl20461/Documents/BristolPhD/SyntheticInSAR/Synthetic_InSAR_image-main-mod/';
SAVEWRAP = 1;
mkdir([outputRoot, 'Suswa/unwrap/turbulent/']);
mkdir([outputRoot, 'Suswa/unwrap/turbulent/']);
if SAVEWRAP == 1
    mkdir([outputRoot, 'Suswa/wrap/turbulent/']);
    mkdir([outputRoot, 'Suswa/wrap/turbulent/']);
end
% parameters
rows = 100;
...
covmodel_type = 0;
N = 10;
halfcrop = floor(500/2); % size of input of Alexnet = 227x227 pixels
...
%             curTur = curTur(round(size(curTur,1)/2) + (-halfcrop:halfcrop),round(size(curTur,2)/2) + (-halfcrop:halfcrop));
            ...
            outputDir = [outputRoot, 'Suswa/unwrap/turbulent/'];
            ...
            if SAVEWRAP == 1
                ...
                outputDir = [outputRoot, 'Suswa/wrap/turbulent/'];
