function [Fig, RMS] = PlotBulkDMR_UNW_Raw(OutputFilePath)
...
invResFiles = dir(OutputFilePath{k});
    ...
    for i=1:length(insar)
        ...
        nexttile
        % Raw data - unwrapped
        scatter(ll_raw(:,1), ll_raw(:,2),0.5,los_raw,'square', 'filled');
    ...
    %     subtitle(['RMSE (mm) = ',num2str(1000*RMSE_LOSRaw,4)],'interpreter','none')
        ...
        nexttile
        % Raw model - unwrapped
        scatter(ll_raw(:,1), ll_raw(:,2),0.5,modLosRaw2,'square', 'filled');
        colormap(cmap2.redToBlue)
        c = max(abs([min(los_raw), max(los_raw)])); % Calculate maximu value for symmetric colormap
        caxis([-c c])
        axis square
    ...
    %     subtitle(['RMSE (mm) = ',num2str(1000*RMSE_ModRaw,4)],'interpreter','none')
        ...
        scatter(ll_raw(:,1), ll_raw(:,2),0.5,ResidualRaw,'square', 'filled');
        colormap(cmap2.redToBlue); 
        c = max(abs([min(los_raw), max(los_raw)])); % Calculate maximu value for symmetric colormap
        caxis([-c c])
        axis square
