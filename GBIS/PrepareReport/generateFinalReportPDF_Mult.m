function ReportFilename = generateFinalReportPDF_Mult(invResFile,burning,Options)
...
for k = 1:length(invpar.model)
    numDisc = 0;
    if contains(invpar.model{k},'SILL') || contains(invpar.model{k},'DIKE')
        ...
        if contains(invpar.model,'SILL')
            if ModelParams(IdxStrike(numDisc),1) >180
                for j = 1:width(ModelParams)
                    ModelParams(IdxStrike(numDisc),j) = ModelParams(IdxStrike(numDisc),j)-180;
                end
            end
        end

        if Options.ModifyStrike(k)==1 % Change strike if width is bigger than length
            if ModelParams(IdxStrike(numDisc),1) + 90 <=180 & ModelParams(IdxStrike(numDisc),1) + 90 >=0
                for j = 1:width(ModelParams)
                    ModelParams(IdxStrike(numDisc),j) = ModelParams(IdxStrike(numDisc),j) + 90;
                end
            else
                for j = 1:width(ModelParams)
                    ModelParams(IdxStrike(numDisc),j) = ModelParams(IdxStrike(numDisc),j) - 90;
        ...
        for j = 1:width(ModelParams)
            Volumes(j) = ModelParams(pIdxL(numDisc),j)*ModelParams(pIdxW(numDisc),j)*ModelParams(pIdxO(numDisc),j);
        end

        ModelParams = [ModelParams(1:pIdxO(numDisc),:);Volumes;ModelParams(pIdxO(numDisc)+1:end,:)];
        if pIdxO == size(Parameter,1)
            Parameter = char(Parameter(1:pIdxO(numDisc),:),'Volume');
        else
            Parameter = char(Parameter(1:pIdxO(numDisc),:),'Volume',Parameter(pIdxO(numDisc)+1:end,:));
        end
    end
end
