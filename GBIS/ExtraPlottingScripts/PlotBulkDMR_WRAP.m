function [Fig, RMS] = PlotBulkDMR_WRAP(OutputFilePath)
...
%% Load InSAR data and results data
    for i=1:length(insar)
        ...
        los = los*4*pi/insar{i}.wavelength;
        modLos2 = modLos2*4*pi/insar{i}.wavelength;
        Residual = Residual*4*pi/insar{i}.wavelength;
        ...
        nexttile
        % Raw data - wrapped
        scatter(ll(:,1), ll(:,2),2,wrapTo2Pi(los),'square', 'filled');
        ...
        nexttile
        % Raw model - wrapped
        scatter(ll(:,1), ll(:,2),2,wrapTo2Pi(modLos2),'square', 'filled');
        colormap(cmap2.Seismo)
        axis square
        xlim([min(ll(:,1)) max(ll(:,1))])
        ylim([min(ll(:,2)) max(ll(:,2))])
        ...
        scatter(ll(:,1), ll(:,2),2,wrapTo2Pi(Residual),'square', 'filled');
        colormap(cmap2.Seismo)
        axis square
        xlim([min(ll(:,1)) max(ll(:,1))])
        ylim([min(ll(:,2)) max(ll(:,2))])
