function [Fig, RMS] = PlotDMR_UNW_RawEdit(OutputFilePath)
...
for i=1:length(insar)
    ...
    nexttile
    % Raw data - unwrapped
    scatter(ll_raw(:,1), ll_raw(:,2),3,los_raw,'square', 'filled');
    colormap(cmap); 
...
%     subtitle(['RMSE (mm) = ',num2str(1000*RMSE_LOSRaw,4)],'interpreter','none')
    ...
    nexttile
    % Raw model - unwrapped
    scatter(ll_raw(:,1), ll_raw(:,2),3,modLosRaw2,'square', 'filled');
    set(gca,'Color',[0.5 0.5 0.5]);
    colormap(cmap)
    c = max(abs([min(los_raw), max(los_raw)])); % Calculate maximu value for symmetric colormap
    caxis([-c c])
...
%     subtitle(['RMSE (mm) = ',num2str(1000*RMSE_ModRaw,4)],'interpreter','none')
    ...
    scatter(ll_raw(:,1), ll_raw(:,2),3,ResidualRaw,'square', 'filled');
    colormap(cmap); 
    c = max(abs([min(los_raw), max(los_raw)])); % Calculate maximu value for symmetric colormap
    caxis([-c c])
    caxis([-0.1 0.1])
