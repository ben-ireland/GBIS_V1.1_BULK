function [Fig, RMS] = PlotDMR_WRAP_Raw(OutputFilePath)
...
for i=1:length(insar)
    ...
    nexttile
    % Raw data - wrapped
    scatter(ll_raw(:,1), ll_raw(:,2),2,wrapTo2Pi(los_raw),'square', 'filled');
    colormap(jet)
    box on
    axis square
...
%set(gca,'YTick',[])
    ...
    nexttile
    % Raw model - wrapped
    scatter(ll_raw(:,1), ll_raw(:,2),2,wrapTo2Pi(modLosRaw2),'square', 'filled');
    colormap(jet)
    box on
    axis square
    xlim([min(ll_raw(:,1)) max(ll_raw(:,1))])
...
%set(gca,'YTick',[])
    ...
    scatter(ll_raw(:,1), ll_raw(:,2),2,wrapTo2Pi(ResidualRaw),'square', 'filled');
    colormap(jet)
    box on
    axis square
    xlim([min(ll_raw(:,1)) max(ll_raw(:,1))])
