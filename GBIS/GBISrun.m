function [OutputFilepath] = GBISrun(inputFileName, insarDataCode, gpsDataFlag, modelCode, nRuns, skipSimulatedAnnealing)
...
%                   'C' for Sun 1969 Penny
%                   'V' for Sun 1969 (Using volume instead of pressure)
%                   'E' for Cervelli 2013 spheroid (using volume)
%                   'R' for Cervelli 2013 spheroid (using pressure)
%                   'A' for Nikkhoo 2017 compound dislocation model
%                   'B' for Nikkhoo 2017 point ellipsoidal cavity model
%
...
%   Modified with "Added" and "Edited" lines by Ben Ireland, University of
%   Bristol, March 2023
...
% Make a folder for ALL inversion results if it does not exist
if ~exist([pwd,'/Inversion_Results'],'dir')
    mkdir(pwd,'/Inversion_Results')
    addpath([pwd,'/Inversion_Results'])
end
...
if gpsDataFlag == 'y' && ~isempty(insarDataCode)
    ...
    %Added to change filename to not overwrite previous runs
    OldsaveName = saveName;
    CurrentFolder = pwd;
    DefaultFile = [CurrentFolder,outputDir,'/',saveName,'/',saveName,'.mat'];
    Overwrite = isfile(DefaultFile);
    n=0
    if Overwrite ==1
        disp 'new file name needed'
        for n=1:1000000
            NewsaveName = [saveName,'_',num2str(n)];
            NewFile = [CurrentFolder,'/',outputDir,'/',OldsaveName,'/',NewsaveName,'.mat'];
            Overwrite = isfile(NewFile);
            if Overwrite==0
                break
                disp(NewFile)
            end
    ...
    outputDir = [outputDir,'/',OldsaveName]; %Modified from 'saveName'
    disp(['Output directory: ', outputDir])
    mkdir(outputDir)
    mkdir([outputDir,'/Figures']) % Create directory for Figures 
    
    if Overwrite ==1
        mkdir([outputDir,'/Figures',num2str(n)]) %Added to allow different figure folders for each run
    end
    
        % Copy and save input file - %Added
    [status,msg]= copyfile(inputFileName, outputDir);
    
    if status ==1
        disp 'Input file saved successfully'
    else
        disp 'Input file not saved - please see any warnings'
    end
...
elseif gpsDataFlag == 'n' && ~isempty(insarDataCode)
    ...
    OldsaveName = saveName
    CurrentFolder = pwd;
    DefaultFile = [CurrentFolder,'/Inversion_Results/',outputDir,'/',saveName,'/',saveName,'.mat']
    DefaultFile2 = [CurrentFolder,'/Inversion_Results/',outputDir,'/',saveName,'/',OldsaveName,'/',saveName,'.mat']
    Overwrite = isfile(DefaultFile) || isfile(DefaultFile2)
    n=0
    if Overwrite ==1
        disp 'new file name needed'
        for n=1:10000000 %Suitably large number
            NewsaveName = [OldsaveName,'_',num2str(n)];
            if length(insar) ==1
                NewFile = [DefaultFile(1:end-13),'/',NewsaveName,'/',NewsaveName,'.mat']
            elseif length(insar)==2
                NewFile = [DefaultFile(1:end-15),'/',NewsaveName,'/',NewsaveName,'.mat']
            else
                NewFile = [DefaultFile(1:end-5),'/',NewsaveName,'/',NewsaveName,'.mat']
            end
            Overwrite = isfile(NewFile)
            if Overwrite==0
                break
    ...
    % Create output directories
    outputDir = [outputDir,'/',OldsaveName]; %Modified from 'saveName'
    disp(['Output directory: ', outputDir,'/',saveName])
    mkdir([pwd,'/Inversion_Results/',outputDir])
    mkdir([pwd,'/Inversion_Results/',outputDir,'/',saveName,'/Figures']) % Create directory for Figures
    ...
    saveName = [saveName,'.mat'];
        ...
        % Copy and save input file - %Added
    [status,msg]= copyfile(inputFileName, [pwd,'/Inversion_Results/',outputDir]);
...
if ~isempty(insarDataCode)
    ...
    disp(' ')
    disp('Ingesting InSAR data and performing subsampling ...')
...
[filepath,name,ext] = fileparts(insar{1}.dataPath);
%% Ingest GPS data
...
for i = 1:invpar.nModels
    ...
    switch modelName
        ...
        case 'C'
            invpar.model{i}='SUNC';    % Analytical Penny-shaped crack of Sun (1969)
        case 'V'
            invpar.model{i}='SUNV';    % Analytical Penny-shaped crack of Sun (1969)
        case 'E'
            invpar.model{i}='CERV';    % Yang implementation of Cervelli (2013) - Volume
        case 'R'
            invpar.model{i}='CERP';    % Yang implementation of Cervelli (2013) - Pressure
        case 'A'
            invpar.model{i}='CDMN';     % Compound dislocation model (Nikkhoo 2017)
        case 'B'
            invpar.model{i}='CDMB';    % CDM (Nikkhoo 2017) simple sphere
        case 'G'
            invpar.model{i}='CDMG';     % CDM (Nikkhoo 2017) sphere with trend/plunge
        case 'I'
            invpar.model{i}='CDMI';     % CDM (Nikkhoo 2017) symmetric sill
        case 'J'
            invpar.model{i}='CDMJ';     % CDM (Nikkhoo 2017) elongated sill
        case 'K'
            invpar.model{i}='CDMK';     % CDM (Nikkhoo 2017) dyke
        case 'L'
            invpar.model{i}='CDML';     % CDM (Nikkhoo 2017) prolate spheroid
        case 'O'
            invpar.model{i}='CDMO';     % CDM (Nikkhoo 2017) oblate spheroid                                            
...
if gpsDataFlag == 'y' && ~isempty(insar)
    ...
    cd([pwd,'/Inversion_Results/',outputDir])
...
Duration = datestr(datenum(0,0,0,0,0,toc),'HH:MM:SS');

%% Save duration and output folder
mkdir('Duration');
save(['Duration/','Duration_',saveName],'Duration');
OutputFilepath = strcat('/Inversion_Results/',outputDir,'/',saveName);
