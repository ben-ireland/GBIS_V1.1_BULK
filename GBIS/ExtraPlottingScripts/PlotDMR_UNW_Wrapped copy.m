...
for i=1:length(insar)
    ...
    nexttile
    % Raw data - unwrapped
    scatter(ll_raw(:,1), ll_raw(:,2),3,los_raw,'.');
    colormap(cmap); 
...
%     subtitle(['RMSE (mm) = ',num2str(1000*RMSE_LOSRaw,4)],'interpreter','none')
    ...
    nexttile
    % Raw model - unwrapped
    scatter(ll_raw(:,1), ll_raw(:,2),3,modLosRaw2,'.');
...
%     subtitle(['RMSE (mm) = ',num2str(1000*RMSE_ModRaw,4)],'interpreter','none')
    ...
    scatter(ll_raw(:,1), ll_raw(:,2),3,ResidualRaw,'.');
    colormap(cmap); 
    c = max(abs([min(los_raw), max(los_raw)])); % Calculate maximu value for symmetric colormap
    caxis([-c c])
    caxis([-0.1 0.1])
...
%     subtitle(['RMSE (mm) = ',num2str(1000*RMSERaw,4)],'interpreter','none')
    ...
    Fig = tl;
    saveas(Fig,[Name,num2str(i),'_Unwrapped.png']);
    ...
    nexttile
    % Raw data - wrapped
    scatter(ll_raw(:,1), ll_raw(:,2),3,wrapTo2Pi(los_raw),'.');
...
%set(gca,'YTick',[])
    ...
    nexttile
    % Raw model - wrapped
    scatter(ll_raw(:,1), ll_raw(:,2),3,wrapTo2Pi(modLosRaw2),'.');
    colormap(cmap2.Seismo)
    ax = gca;
    ax.Layer = 'top';
    ax.Box = 'on';
...
%set(gca,'YTick',[])
    ...
    scatter(ll_raw(:,1), ll_raw(:,2),3,wrapTo2Pi(ResidualRaw),'.');
    colormap(cmap2.Seismo)
    ax = gca;
    ax.Layer = 'top';
    ax.Box = 'on';
...
%     subtitle(['RMSE (mm) = ',num2str(1000*RMSERaw,4)],'interpreter','none')
    ...
    %set(gca,'YTick',[])
    Fig = tl;
    saveas(Fig,[Name,num2str(i),'_Wrapped.png']);
end
end