function [Sill,Range,Nugget] = fitVariogram_AOI(inpFile,inputFile, wavelength)
...
%
%   inpFile:    Path to .inp file containing the coordinates of the fine
%               subsampling box i.e. containing deformation source
%
...
hold on
pgon = polyshape([geo.FineSamplingBox(1) geo.FineSamplingBox(1) geo.FineSamplingBox(3)...
    geo.FineSamplingBox(3)],[geo.FineSamplingBox(2) geo.FineSamplingBox(4) geo.FineSamplingBox(4)...
    geo.FineSamplingBox(2)]);
plot(pgon)
axis xy
axis equal
...
%% Identify data outside of Fine Sampling Box
        ...
        disp(['Lon limits = ',num2str(geo.FineSamplingBox(1)),', ',num2str(geo.FineSamplingBox(3))])
        disp(['Lat limits = ',num2str(geo.FineSamplingBox(4)),', ',num2str(geo.FineSamplingBox(2))])
        
        in = isinterior(pgon,insarData.Lon,insarData.Lat);
        ixSubset = find(in == 0);
