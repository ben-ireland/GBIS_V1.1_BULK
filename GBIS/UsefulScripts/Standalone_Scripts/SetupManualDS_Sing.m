...
VolcName = {'2026_Dabbahu_Dsc'};

% Options for manual AOI definition
MANUAL_Options.Manual_Suffix = '_Uplift'; % Check underscore against shapefile name.
...
Options.SS_Factor = 50; % Coarse downsampling factor i.e. anywhere outside the AOI is averaged over every x by x pixels
Options.SS_FactorF = 30; % Fine downsampling factor i.e. anywhere inside the AOI is averaged over every x by x pixels
...
Options.Max_nPix = 4000; % Maximum number of pixels for a downsampled imaged (if Options.Adjust_SS_Factor == 1)
