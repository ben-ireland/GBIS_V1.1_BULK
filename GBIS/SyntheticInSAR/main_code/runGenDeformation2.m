...
Mogi.Volume = 3.95*1e6;               %Volume in m^3
...
if Source_Type == 4
    count = 1;
    Incidence = 39.5879  %[3 8 12 29 45 140 170 210 330 350] Suswa = 39.5879
        incidenceName = ['incidence',sprintf('%03d',Incidence)];
        Heading = 12.1221 %5:40:330 Suswa = -12.1221
            headingName = [incidenceName, '_heading',sprintf('%03d',Heading)];
            %for Volume = 3.95 %[6 6.5 6.7 7 7.2 7.5]+0.2 Suswa = 3.95
                Volume = 3.95
                Mogi.Volume = Volume*10^6
%                 Mogi.Volume = 10^Volume;                    %dip in degrees
                VolumeName = [headingName, '_vol1e',sprintf('%0.1f',Volume)];
                depth_range = 4.923
                depth = depth_range; %Suswa best-fit value
%                 if Volume<=6
%                     depth_range = [1.5 2];
%                 elseif Volume<=6.5
...
%                         los_grid = los_grid(round(size(los_grid,1)/2) + (-halfcrop:halfcrop),round(size(los_grid,2)/2) + (-halfcrop:halfcrop));
                        ...
                        Test(n) = (range(los_grid(:)) > 10)&&(range(los_grid(:)) < 20); %added
                        Ranges(n) = range(los_grid(:));
                        n = n+1; %added
%                         if (range(los_grid(:)) > 10)&&(range(los_grid(:)) < 20)%60)
%                         if (range(los_grid(:)) > 0.1)
%                               los_grid = 100*los_grid; %added
                        outputDirUnwrap = [outputRoot, '/Suswa/unwrap/deform/'];
                        save([outputDirUnwrap, allName, '.mat'], 'los_grid');
                        if SAVEWRAP == 1
                            outputDirWrap = [outputRoot, '/Suswa/wrap/deform/'];
                            los_grid_wrap = wrapTo2Pi(los_grid)-pi;
                            los_grid_wrap = (los_grid_wrap-min(los_grid_wrap(:)))/range(los_grid_wrap(:));
                            imwrite(los_grid_wrap, [outputDirWrap, allName, '.png']);
                        end
