function FullTable = Step9_GenerateReportsTable(FullTable,OutputFilepath,NumFrames,VolcNames,VolcName,Options)

    if Options.PennyComparison ==1 && NumFrames >1
        disp('Generating Mogi reports (~2 mins)')
        SignalLocationGBIS = SignalLocationFromResults(OutputFilepath{1}, VolcName);
        Area = ExtractGBISSignalArea(OutputFilepath{1},Options.AreaCutoff);
        SignalLocationGBIS.AreaKm2 = Area;
        ReportFilePath = generateFinalReportPDF(OutputFilepath{1},SignalLocationGBIS,Options.Burnin);
        Report = CreatePDF_Report(ReportFilePath,OutputFilepath{1},VolcNames,Options);

        disp('Generating Penny reports (~2 mins)')
        SignalLocationGBISPen = SignalLocationFromResults(OutputFilepath{2}, VolcName);
        Area = ExtractGBISSignalArea(OutputFilepath{2},Options.AreaCutoff);
        SignalLocationGBISPen.AreaKm2 = Area;
        ReportFilePathPen = generateFinalReportPDF(OutputFilepath{2},SignalLocationGBISPen,Options.Burnin);
        ...
        disp('Generating reports (~2 mins)')
        SignalLocationGBIS = SignalLocationFromResults(OutputFilepath{1}, VolcNames);
        Area = ExtractGBISSignalArea(OutputFilepath{1},Options.AreaCutoff);
        SignalLocationGBIS.AreaKm2 = Area;
        ReportFilePath = generateFinalReportPDF(OutputFilepath{1},SignalLocationGBIS,Options.Burnin);
        Report = CreatePDF_Report(ReportFilePath,OutputFilepath{1},VolcNames,Options);
    ...
    if length(OutputFilepath) > 2
        for k = 3:length(OutputFilepath)
            SignalLocationGBIS = SignalLocationFromResults(OutputFilepath{k}, VolcName);
            Area = ExtractGBISSignalArea(OutputFilepath{1},Options.AreaCutoff);
            SignalLocationGBIS.AreaKm2 = Area;
            ReportFilePath = generateFinalReportPDF(OutputFilepath{k},SignalLocationGBIS,Options.Burnin);
