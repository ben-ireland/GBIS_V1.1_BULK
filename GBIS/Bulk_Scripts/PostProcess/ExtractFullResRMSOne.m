function  RMS = ExtractFullResRMSOne(OutputFilePath,DS)
...
for i=1:length(insar)
    if DS ==1
        ...
        loadedData = load(insar{i}.rawDataPath);
    end
    ...
    %Downsampled data
    % RMSE of data
    AdjLos = los - mean(los(:)); % Demean los
    RMSE_LOS = sqrt((sum(los.^2))./nObs2);
    % RMSE of model
    RMSE_Mod = sqrt((sum(modLos2.^2))./nObs2);
    % RMSE of residual
    Residual = los-modLos2;
    AdjResidual = AdjLos-modLos2;

    RMSE = sqrt((sum(Residual.^2))./nObs2);
    RMSE_Reduction = ((RMSE_LOS-RMSE)/RMSE_LOS)*100;
    AdjRMSERes = sqrt((sum(AdjResidual.^2))./nObs2);
    AdjRMSE_LOS = sqrt((sum(AdjLos.^2))./nObs2);
    AdjRMSE_Reduction = ((AdjRMSE_LOS-AdjRMSERes)/AdjRMSE_LOS)*100;
    ...
    RMS.AdjLOS = AdjRMSE_LOS;
    RMS.AdjRes = AdjRMSERes;
    RMS.AdjReduction = AdjRMSE_Reduction;
