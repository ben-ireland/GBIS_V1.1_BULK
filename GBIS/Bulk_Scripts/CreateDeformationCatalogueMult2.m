...
for m = 1:length(invpar.model)
    for k = 1:NumFrames
        ...
        ModIdxRange = invResults.model.mIx(m):invResults.model.mIx(m+1)-1;
        ...
        if contains(invpar.model(m),'SILL') || contains(invpar.model(m),'DIKE')
            ...
            % Modify strike if width is bigger than length
    ...
    if exist('Model')==1 && exist('SignalLocation')==1
        Table{m} = horzcat(Name,TemporalTab,SpatialTab,SourceParameters);
    else
        Table{m} = SourceParameters;
    end
    % Changing data types so tables can be concatenated
    Table{m}.TComments = {Table{m}.TComments};
    Table{m}.SComments = {Table{m}.SComments};
    Table{m}.TComments = cellfun(@char, Table{m}.TComments, 'UniformOutput', false);
    Table{m}.SComments = cellfun(@char, Table{m}.SComments, 'UniformOutput', false);
