function Location = Step2_SlidingWindowClusteringSTD(LastStep, VolcName, Options)
    ...
    f2 = figure()
    subplot(1,2,1)
    imagesc(LastStep.*rad2m,'AlphaData',~isnan(LastStep).*0.7)
    colormap(cbrewer('RdBu'))
    %caxis([-0.1 0.1])
    axis image
    hold on
    cRamp = gray(length(WindowSize));
    scatter(maxPixelCol,maxPixelRow,(maxMeanValue./max(maxMeanValue)*100),cRamp(1:length(maxPixelCol),:),'filled')
    xticklabels({''})
    yticklabels({''})
    set(gca,'XTick',[])
    set(gca,'YTick',[])
    hold off
    subplot(1,2,2)
    gscatter(maxPixelCol,maxPixelRow,idx2,[],'o',[],'doleg','off','filled')
    xlim([0,size(LastStep,1)])
    ylim([0,size(LastStep,2)])
    set(gca,'YDir','reverse')
    axis square
    hold on
    plot(pgon,"EdgeColor",'k','FaceAlpha',0.4,'FaceColor','k')
    xlabel({''});
    ylabel({''});
    xticklabels({''})
    yticklabels({''})
    ...
    saveas(f,[pwd,'/SignalLocation/STD_',VolcName,'_SlidingWindow_Clustering_',num2str(Options.SW_DBSCAN_Eps),'_',num2str(Options.SW_DBSCAN_MinPts),'_Box_',num2str(Options.SW_MinBBSize),'.png'])
    saveas(f2,[pwd,'/SignalLocation/STD_',VolcName,'_SlidingWindow_Clustering_',num2str(Options.SW_DBSCAN_Eps),'_',num2str(Options.SW_DBSCAN_MinPts),'_Box_',num2str(Options.SW_MinBBSize),'clean.png'])
    save([pwd,'/SignalLocation/STD_',VolcName,'_SlidingWindow_Clustering_',num2str(Options.SW_DBSCAN_Eps),'_',num2str(Options.SW_DBSCAN_MinPts),'_Box_',num2str(Options.SW_MinBBSize),'.mat'],'Location','Points2','idx2','corePts2','WindowSize');
end