function [Fig, RMS] = PlotDMR_WRAP(OutputFilePath)
...
for i=1:length(insar)
    ...
    nexttile
    % Raw data - wrapped
    scatter(ll(:,1), ll(:,2),5,wrapTo2Pi(los),'square', 'filled');
    ...
    nexttile
    % Raw model - wrapped
    scatter(ll(:,1), ll(:,2),5,wrapTo2Pi(modLos2),'square', 'filled');
    colormap(cmap2.Seismo)
    axis square
    xlim([min(ll(:,1)) max(ll(:,1))])
    ylim([min(ll(:,2)) max(ll(:,2))])
    ...
    scatter(ll(:,1), ll(:,2),5,wrapTo2Pi(Residual),'square', 'filled');
    colormap(cmap2.Seismo)
    axis square
    xlim([min(ll(:,1)) max(ll(:,1))])
    ylim([min(ll(:,2)) max(ll(:,2))])
