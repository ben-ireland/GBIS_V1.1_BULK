function Model = FitTimeseriesFunctions(LOS, days, FileInfo, SignalLocation, Name)
...
Model.Name = Name;
TS = double(squeeze(LOS(SignalLocation(1),SignalLocation(2),:))); % Lon, Lat
figure()
...
fitParams = lsqcurvefit(sigmoidalFit, initialGuess, days, TS,[],[]);
...
if LinAdjR2<0.5 && SigAdjR2>0.5 % If Linear R2 is <0.5
    Model.Notes = 'Linear R2 <0.5';
elseif LinAdjR2>0.5 && SigAdjR2<0.5 % If Sigmoid R2 is <0.5
    Model.Notes = 'Sigmoidal R2 <0.5';
end

%% Fit a hybrid trendline and compute the RSS (?) - how to identify a seasonal component?
if LinAdjR2<0.5 && SigAdjR2<0.5
    % Try to remove a seasonal component?
    Model.Type = 'Other';
    Model.Notes = 'R2 <0.5 for linear and sigmoid';
    Model.Flag = 1;
    Model.AIC = NaN;
    Model.R2 = 0;
