    ...
    if NumFrames>1
        ...
        if Options.PennyComparison ==1
            ...
            if Options.PennyType ==1
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Penny source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','P',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            elseif Options.PennyType ==2
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Penny source (Sun 1969 solution with pressure)')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','C',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            elseif Options.PennyType ==3
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Penny source (Sun 1969 solution with volume)')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','V',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
        ...
        if Options.SillComparison ==1
            ...
            if Options.SeedingRun ==1
                InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                disp('Seeding run for sill source')
                SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','S',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                cd(startDir)
                InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
            end
        ...
        if Options.YangComparison ==1
            ...
            if Options.YangType ==1
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Yang source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','Y',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            elseif Options.YangType ==2
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Yang source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','E',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            elseif Options.YangType ==3
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Yang source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','R',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
        ...
        if Options.DykeComparison ==1
            ...
            if Options.SeedingRun ==1
                InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                disp('Seeding run for dyke source')
                SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','D',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                cd(startDir)
                InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
            end
        ...
        if Options.CDMComparison ==1
            if ismember(1,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','A',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(2,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','B',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(3,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','G',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(4,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','I',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(5,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','J',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(6,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','K',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(7,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','L',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(8,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1,2],'n','O',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
        ...
        elseif Options.SourceType==2
            if Options.SeedingRun ==1
                InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                disp('Seeding run for Penny source')
                SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','P',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                cd(startDir)
                InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
            end
        ...
        if Options.SillComparison ==1
            ...
            if Options.SeedingRun ==1
                InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                disp('Seeding run for Sill source')
                SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','S',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                cd(startDir)
                InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
            end
        ...
        if Options.PennyComparison ==1
            ...
            if Options.PennyType ==1
                ...
                OutputFilepaths{end+1} = GBISrun(InpFilePath,[1],'n','P',(Options.nRuns),Options.skipSimulatedAnnealing);
            elseif Options.PennyType ==2
                disp('Running GBIS for Penny source (Sun 1969 solution with pressure) slower than Mogi')
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Penny source (Sun 1969 solution with pressure)')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','C',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
                OutputFilepaths{end+1} = GBISrun(InpFilePath,[1],'n','C',(Options.nRuns),Options.skipSimulatedAnnealing);
            elseif Options.PennyType ==3
                disp('Running GBIS for Penny source (Sun 1969 solution with volume) slower than Mogi')
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Penny source (Sun 1969 solution with volume)')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','V',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
                OutputFilepaths{end+1} = GBISrun(InpFilePath,[1],'n','V',(Options.nRuns),Options.skipSimulatedAnnealing);
            end
            close all
            toc
        end
        ...
        if Options.YangComparison ==1
            ...
            if Options.YangType ==1
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Yang source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','Y',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
                OutputFilepaths{end+1} = GBISrun(InpFilePath,[1],'n','Y',(Options.nRuns),Options.skipSimulatedAnnealing);
            elseif Options.YangType ==2
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Yang source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','E',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
                OutputFilepaths{end+1} = GBISrun(InpFilePath,[1],'n','E',(Options.nRuns),Options.skipSimulatedAnnealing);
            elseif Options.YangType ==3
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for Yang source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','R',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
                OutputFilepaths{end+1} = GBISrun(InpFilePath,[1],'n','R',(Options.nRuns),Options.skipSimulatedAnnealing);
            else
                error('Options.YangType can only have values 1, 2 or 3')
            end
        end
        ...
        if Options.DykeComparison ==1
            ...
            if Options.SeedingRun ==1
                InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                disp('Seeding run for Dyke source')
                SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','D',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                cd(startDir)
                InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
            end
        ...
        if Options.CDMComparison ==1
            if ismember(1,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','A',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(2,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','B',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(3,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','G',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(4,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','I',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(5,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','J',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(6,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','K',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(7,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','L',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
            ...
            if ismember(8,Options.CDMGeometry)
                ...
                if Options.SeedingRun ==1
                    InpFilePathSeed = strcat(InpFilePath(1:end-4),'_Seed.inp');
                    disp('Seeding run for CDM source')
                    SeedFilepath = GBISrun(InpFilePathSeed,[1],'n','O',(Options.SeedingnRuns),Options.skipSimulatedAnnealing);
                    cd(startDir)
                    InpFilePath = ProcessSeedingRun(InpFilePath,SeedFilepath,Options);
                end
