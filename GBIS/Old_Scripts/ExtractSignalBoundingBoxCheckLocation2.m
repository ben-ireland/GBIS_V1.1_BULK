function [BoundingBox, Image, OtsuFigure, SignalLocation, largest_component_mask] = ExtractSignalBoundingBoxCheckLocation2(BB_Shape, LastStep, numlevels, VolcName, SpatialRes, lon, lat, Location, Options)
...
duplicateMethod = 1; % 1== try second largest displacement settings. 2== try second largest connected region. 3== try both at once
...
LastStep = IFG_Region_Shrink(LastStep,500,3,VolcName,0);
...
for i=1:numlevels
    ...
    for j=1:length(levelnum)+1
        ...
        if length(LastStep(labels==j))>200 %Ignore classes with <200 pixels
            ...
            stats(stats.Area < 100,:)=[]; % Delete connected components with less than 100 pixels
            ...
            for k = 1:size(stats,1)
                ...
                if Overlap(k) == 1
                    ...
                    break
                    %SignTest = mean(LastStep(MaskAreaBefore==1),'omitnan');
                    %if sign(SignTest) == sign(TestLOS)
                    %    AvgLOSMask(i,j) = mean(abs(LastStep(MaskAreaBefore==1)),'omitnan');
                    %    MaskCC{i,j} = MaskAreaBefore;
                    %    break
                    %else
    ...
    if i==numlevels
        ...
        % Iteratively change the size of the bounding box so perimeter contains above noise values
        EdgePixels = bwperim(BoundingBoxMask,8);
        MeanEdgeValsOrig = mean(abs(LastStep(EdgePixels)),'omitnan');
        Threshold = 0.012;

        if MeanEdgeValsOrig > Threshold
            MeanEdgeVals = MeanEdgeValsOrig;
            d = 0;
            while MeanEdgeVals > Threshold
                d = d+1
                BoundingBoxMask = imdilate(BoundingBoxMask,strel('disk', 1));
                NewEdgePixels = bwperim(BoundingBoxMask,8);
                MeanEdgeVals = mean(abs(LastStep(NewEdgePixels)),'omitnan');
                if d==50
                    break
                end
            end
        elseif MeanEdgeValsOrig < Threshold
            MeanEdgeVals = MeanEdgeValsOrig;
            d = 0;
            while MeanEdgeVals < Threshold
                d = d+1
                BoundingBoxMask = imerode(BoundingBoxMask,strel('disk', 1));
                NewEdgePixels = bwperim(BoundingBoxMask,8);
                MeanEdgeVals = mean(abs(LastStep(NewEdgePixels)),'omitnan');
                if d==50
                    break
        ...
        Boundaries = bwboundaries(BoundingBoxMask);
        BoundingBox = Boundaries{1};
        BoundingBox = polyshape(BoundingBox(:,1),BoundingBox(:,2));
