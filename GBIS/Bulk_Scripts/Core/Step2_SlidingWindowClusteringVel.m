function Location = Step2_SlidingWindowClusteringVel(LastStep, VolcName, Options)
    ...
    axis image
    hold on
    cRamp = copper(length(WindowSize));
    ...
    axis square
    hold on
    plot(pgon,"EdgeColor",'k','FaceAlpha',0.35,'FaceColor','k')
    xlabel({''});
    ylabel({''});
    xticklabels({''})
    yticklabels({''})
    ...
    save([pwd,'/SignalLocation/',VolcName,'_SlidingWindow_Clustering_',num2str(Options.SW_DBSCAN_Eps),'_',num2str(Options.SW_DBSCAN_MinPts),'_Box_',num2str(Options.SW_MinBBSize),'.mat'],'Location','Points2','idx2','corePts2','WindowSize');
end