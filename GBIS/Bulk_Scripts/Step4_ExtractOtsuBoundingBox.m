function [BoundingBox, Image, OtsuFigure, SignalLocation, largest_component_mask] = Step4_ExtractOtsuBoundingBox(LastStep, Location, VolcName, lon, lat, Options)
...
for i=1:numlevels
    ...
    for j=1:length(levelnum)+1
        ...
        if length(LastStep(labels==j))>Options.Otsu_MinClassSize %Ignore classes with <200 pixels
            ...
            for k = 1:size(stats,1)
                ...
                %MaskAreaBefore = MaskAreaBefore ==1 & LastStep~=0;
    ...
    if i==numlevels
        ...
        elseif BB_Shape ==2
            ...
            if maskArea > Options.Otsu_MaxMaskArea
                PgonBuffer = polybuffer(newCoords,'points',Options.Otsu_Buffer_Large);
            else
                PgonBuffer = polybuffer(newCoords,'points',(10*(maskArea^(1/3))));
            end
