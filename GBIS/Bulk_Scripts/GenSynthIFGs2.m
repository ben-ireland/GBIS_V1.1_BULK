function [SynData] = GenSynthIFGs2(Deformation, Stratified, Turbulent, Wrapped, Quake, Dyke, Sill, Mogi, Penny, Heading, Incidence, wavelength, Source_Type, k, ID, CoherenceMask)   
...
if Deformation == 1
    ...
    if ~exist([pwd,'/SyntheticIFGs/',Source],'dir')
        mkdir([pwd,'/SyntheticIFGs'],Source)
        addpath([pwd,'/SyntheticIFGs/',Source])
    end
...
if ~exist([pwd,'/SyntheticIFGs/',Source,'/',ID,num2str(k)],'dir')
    mkdir([pwd,'/SyntheticIFGs/',Source],[ID,num2str(k)])
    addpath([pwd,'/SyntheticIFGs/',Source,'/',ID,num2str(k)])
end

savedir = [pwd,'/SyntheticIFGs/',Source,'/',ID,num2str(k),'/'];
imwrite(los_grid,[savedir,'los',num2str(k),'.png']);
imwrite(atmo,[savedir,'atmo',num2str(k),'.png']);
imwrite(curTur,[savedir,'cur',num2str(k),'.png']);
imwrite(combined,[savedir,'combined',num2str(k),'.png']);
save([savedir,'unwrapped_los','_',Source,'_',Deform,'_',Strat,'_',Turb,'_',num2str(k),'.mat'],'los_grid','atmo','curTur','combined');
...
colormap("parula")
axis square
caxis([min(combined(:)), max(combined(:))]);
c = colorbar;
...
saveas(gcf,[savedir,'unwrappedFig_',num2str(k),'.png']);
...
if Wrapped == 1
    ...
    sgtitle(['Source type: ',Source,'   ','Volume (m^3): ',sprintf('%2e',Mogi.Volume),'      ',' Depth (km): ',num2str(Mogi.Depth)]);
    saveas(gcf,[savedir,'wrappedFig_',num2str(k),'.png']);
    
    imwrite(los_grid_wrap,[savedir,'los_wrap',num2str(k),'.png']);
    imwrite(atmo,[savedir,'atmo_wrap',num2str(k),'.png']);
    imwrite(curTur,[savedir,'curTur_wrap',num2str(k),'.png']);
    imwrite(combinedWrap,[savedir,'combined_wrap',num2str(k),'.png']);
    save([savedir,'wrapped_los','_',Source,'_',Deform,'_',Strat,'_',Turb,'_',num2str(k),'.mat'],'los_grid_wrap','atmo','curTur','combinedWrap');
end
...
SynData.filepath = [savedir,'unwrapped_los','_',Source,'_',Deform,'_',Strat,'_',Turb,'_',num2str(k),'.mat'];
