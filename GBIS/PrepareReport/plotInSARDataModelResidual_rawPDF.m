function name_raw = plotInSARDataModelResidual_rawPDF(insar, geo, invpar, invResults, modelinput, saveName, saveflag)
...
%     Apply bounding box and remove data points outside the AOI
    ...
    [path_raw, name_raw{i}, ext_raw] = fileparts(insar{i}.rawDataPath);
   ...
   if i > 1
    ...
    img = getframe(gcf);
    if saveflag=='y'
        imwrite(img.cdata,[outputDir,'/','Summary_Figures/',inputFileN,'/',saveName,'/InSAR_Data_Model_Residual_',name_raw{i},num2str(i),'.png']); %Edited
        
        save([outputDir,'/Residual/Residual','raw_',name_raw{i},saveName,'.mat'],'residual_raw','xy_raw'); %Added
        save([outputDir,'/Model/Model','raw_',name_raw{i},saveName,'.mat'],'modLos_raw','xy_raw'); %Added
    end
